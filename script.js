const menuToggle = document.querySelector('.menu-toggle input');
const nav = document.querySelector('nav ul');

menuToggle.addEventListener('click', function(){
    nav.classList.toggle('slide');
})

// show product
const rowProduct = document.getElementById('men-product');
var html = '';

var dataProduct = [
    {
        "title": "Product A",
        "price": 100000,
        "image": "img/portofolio/1.jpg"
    },
    {
        "title": "Product B",
        "price": 110000,
        "image": "img/portofolio/2.jpg"
    },
    {
        "title": "Product C",
        "price": 120000,
        "image": "img/portofolio/3.jpg"
    },
    {
        "title": "Product D",
        "price": 130000,
        "image": "img/portofolio/4.jpg"
    },
    {
        "title": "Product E",
        "price": 100000,
        "image": "img/portofolio/1.jpg"
    },
    {
        "title": "Product F",
        "price": 110000,
        "image": "img/portofolio/2.jpg"
    },
    {
        "title": "Product G",
        "price": 120000,
        "image": "img/portofolio/3.jpg"
    },
    {
        "title": "Product H",
        "price": 130000,
        "image": "img/portofolio/4.jpg"
    },
    {
        "title": "Product I",
        "price": 100000,
        "image": "img/portofolio/1.jpg"
    },
    {
        "title": "Product J",
        "price": 110000,
        "image": "img/portofolio/2.jpg"
    },
    {
        "title": "Product K",
        "price": 120000,
        "image": "img/portofolio/3.jpg"
    },
    {
        "title": "Product L",
        "price": 130000,
        "image": "img/portofolio/4.jpg"
    }
];

for(product of dataProduct){
    html += '<div class="col-men-product">'+
        '<img src='+product.image+' width="350px" height="300px" >'+'<BR>'+
        'Title : '+product.title+'<BR>'+
        +product.price+'<BR>'+
    '</div>';
}
console.log(rowProduct);

rowProduct.innerHTML = html;
const price = querySelector('price');
price.style.color = 'red';